...
from torchtune.models.phi3.long_scaled_rope import LongScaledRotaryPositionalEmbeddings
...
"""
Component builders for the Phi-3 model and popular variants such as LoRA.
...
def phi3(
    *,
    ...
    rope_scaling_policy: Optional[str] = None,
    rope_scaling_factors: Optional[Tuple[List[float], List[float]]] = None,
    intermediate_dim: Optional[int] = None,
    norm_eps: float = 1e-6,
) -> TransformerDecoder:
    """
    Build the decoder associated with the Phi-3 model. This includes:
    - Token embeddings
    ...
    Returns:
        TransformerDecoder: Instantiation of Phi-3 model.
    """
    head_dim = embed_dim // num_heads
    num_kv_heads = num_kv_heads if num_kv_heads else num_heads
    if rope_scaling_policy is not None:
        assert rope_scaling_factors is not None, f"Must provide scaling factors if also requesting rope_scaling_policy = {rope_scaling_policy}"
        rope = LongScaledRotaryPositionalEmbeddings(
            dim=head_dim,
            short_factor=rope_scaling_factors[0],
            long_factor=rope_scaling_factors[1],
            max_seq_len=max_seq_len,
            original_max_seq_len=original_max_seq_len,
            base=rope_base,
            scaling_policy=rope_scaling_policy,
        )
    else:
...
def phi3_mlp(dim: int, hidden_dim: int) -> FeedForward:
    """
    Build the MLP layer associated with the Phi-3 model.
    """
